...
== Source file context for tree position ==
   ...
   152     }
   153     def clearLines(currBoard : Map[Point, CellType], linesToRemove : Map[Int, Boolean]) : Map[Point, CellType] = {
   154         (1 to gridDims.height).foldLeft(currBoard) {
   155             if(linesToRemove(_CURSOR_y)) (boardToUpdate, y) => clearOneLine(boardToUpdate,y)
   156         }
   157     }
   158 
...
// use integers to check for specific bound behaviour
    ...
    def newPiecePlacement() : GameState = {
        ...
        val preClearBoard : Map[Point,CellType] = reloadBoard(newPieceAbsPos, piece.blockType)
        val linesToRemove : Map[Int, Boolean] = checkAllLines(preClearBoard)
        val nBoard : Map[Point, CellType] = clearLines(preClearBoard, linesToRemove)
    ...
    def checkAllLines(newBoard : Map[Point, CellType]) : Map[Int,Boolean] = {
        val linesToRemove : Map[Int, Boolean] = (for {
            row <- 0 until gridDims.height
        } yield row -> checkOneLine(newBoard, row)).toMap
        println(linesToRemove)
        return linesToRemove
    }
    def clearOneLine(currBoard : Map[Point, CellType], col : Int) : Map[Point, CellType] = {
        val boardToChange : Map[Point, CellType] = currBoard.filter{case (Point(_,y),_) => col == y}
        val newBoard : Map[Point, CellType]= boardToChange.map{
            case(Point(x,y), cellType) => val aboveCellType = boardToChange.get(Point(x,y - 1)).getOrElse(cellType)
            Point(x,y) -> aboveCellType
         }
        return newBoard
    }
    def clearLines(currBoard : Map[Point, CellType], linesToRemove : Map[Int, Boolean]) : Map[Point, CellType] = {
        (1 to gridDims.height).foldLeft(currBoard) {
            if(linesToRemove(@@y)) (boardToUpdate, y) => clearOneLine(boardToUpdate,y)
        }
    }
