...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Function(value $anonfun)
       tree position: line 139 of file://<HOME>/Uni/2024-2025/Period%201/OOFP/OOFP-Tetris-2024/src/tetris/logic/GameState.scala
            tree tpe: ((tetris.logic.Point, tetris.logic.CellType)) => (tetris.logic.Point, tetris.logic.CellType)
              symbol: value $anonfun
   symbol definition: val $anonfun: <notype> (a TermSymbol)
      symbol package: tetris.logic
       symbol owners: value $anonfun -> value newBoard -> method clearOneLine -> class GameState
...
== Source file context for tree position ==
   ...
   136     }
   137 
   138     def clearOneLine(currBoard : Map[Point, CellType], col : Int, cond : (Point) => Boolean) : Map[Point, CellType] = {
   139         val newBoard : Map[Point, CellType]= currBoard.map{
   140             case(point @ Point(x,y), cellType) => 
   141             if(checkLine(point_CURSOR_.)){
   142                 println("Removing line:", y)
...
// use integers to check for specific bound behaviour
    ...
    def clearLines(currBoard : Map[Point, CellType],col : Int): Map[Point, CellType] = {
        if(col > gridDims.height - 1){
            ...
            val cond : Point => Boolean = p => p.y > col && checkOneLine(currBoard, col)
            val nBoard : Map[Point, CellType] = clearOneLine(currBoard,col, cond)
        }
    }
