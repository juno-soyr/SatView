...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Ident(y)
       tree position: line 153 of file://<HOME>/Uni/2024-2025/Period%201/OOFP/OOFP-Tetris-2024/src/tetris/logic/GameState.scala
            tree tpe: <error>
...
== Source file context for tree position ==
   ...
   150     }
   151     def clearLines(currBoard : Map[Point, CellType]): Map[Point, CellType] = {
   152         (0 to gridDims.height - 1).foldLeft(currBoard) {
   153             val cond : Point => Boolean = p => p.y > _CURSOR_y && checkOneLine(currBoard, y)
   154             (boardToUpdate, y) => clearOneLine(boardToUpdate,y, cond)
   155         }
   156     }
...
// use integers to check for specific bound behaviour
    ...
    def clearLines(currBoard : Map[Point, CellType]): Map[Point, CellType] = {
        (0 to gridDims.height - 1).foldLeft(currBoard) {
            val cond : Point => Boolean = p => p.y > @@y && checkOneLine(currBoard, y)
            (boardToUpdate, y) => clearOneLine(boardToUpdate,y, cond)
        }
    }
