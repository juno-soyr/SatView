...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Ident(p)
       tree position: line 81 of file://<HOME>/Uni/2024-2025/Period%201/OOFP/OOFP-Tetris-2024/src/tetris/logic/GameState.scala
            tree tpe: <error>
              symbol: value p
   symbol definition: p: <error> (a TermSymbol)
      symbol package: tetris.logic
       symbol owners: value p -> value $anonfun -> value newPieceAbsPos -> method moveLeft -> class GameState
           call site: <none> in <none>

== Source file context for tree position ==

    78     }
    79     def moveLeft() : GameState = {
    80         val newPiecePos : (Int, Int) = (piecePos._1 - 1, piecePos._2)
    81         val newPieceAbsPos : List[Point] = _CURSOR_newPiece.struct.map(p => Point(p.x + piecePos._1, p.y + piecePos._2))
    82         
    83         newGameState(newPiecePos = newPiecePos)
    84     }
...
class GameState(val gridDims : Dimensions, val randomGen : RandomGenerator,val piece : Tetronimo,val piecePos : (Int, Int) = (0,0), val board : Map[Point, CellType] ,val newPieceFlag : Boolean = false){
    ...
    def boundCheck(currPos : List[Point]) : Int = {
        for(p <- currPos){
            if (p.x > gridDims.width || p.x < 0){
                return 2
            }else if(board(p) != Empty || p.y > gridDims.height + 1 ){
                return 3
            }
        }
        return -1
...
// use integers to check for specific bound behaviour
    ...
    def rotatePieceLeft() : GameState = {
        ...
        if(boundCheck(newPieceAbsPos) != 2){
