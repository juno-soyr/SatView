...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Ident(_CURSOR_newPiecePos)
       tree position: line 88 of file://<HOME>/Uni/2024-2025/Period%201/OOFP/OOFP-Tetris-2024/src/tetris/logic/GameState.scala
            tree tpe: <error>
...
== Source file context for tree position ==
    ...
    85     }
    86     def moveDown() : GameState = {
    87         val nPiecePos : (Int, Int) = (piecePos._1, piecePos._2 + 1)
    88         val newPieceAbsPos : List[Point] = piece.struct.map(p => Point(p.x + nPiecePos._1, p.y + _CURSOR_newPiecePos._2))
    89         if(boundCheck(newPieceAbsPos)){
    90             return newGameState(newPiecePos = nPiecePos)
    91         }
...
class GameState(val gridDims : Dimensions, val randomGen : RandomGenerator,val piece : Tetronimo,val piecePos : (Int, Int) = (0,0), val board : Map[Point, CellType] ,val newPieceFlag : Boolean = false){
    ...
    def moveDown() : GameState = {
        ...
        val newPieceAbsPos : List[Point] = piece.struct.map(p => Point(p.x + nPiecePos._1, p.y + @@newPiecePos._2))
