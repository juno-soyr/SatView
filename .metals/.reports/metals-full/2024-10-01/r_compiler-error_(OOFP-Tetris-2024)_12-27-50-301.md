...
== Source file context for tree position ==
    ...
    37         val newPieceAbsPos : List[Point] = _CURSOR_newPiece.struct.map(p => Point(p.x + newPiecePos._1, p.y + newPiecePos._2))
...
// use integers to check for specific bound behaviour
    ...
    def newPiecePlacement() : GameState = {
        val newPieceAbsPos : List[Point] = @@newPiece.struct.map(p => Point(p.x + newPiecePos._1, p.y + newPiecePos._2))
