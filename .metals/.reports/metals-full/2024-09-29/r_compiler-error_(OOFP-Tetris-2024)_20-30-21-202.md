...
### scala.reflect.internal.FatalError: 
  ThisType(method pointInPiece) for sym which is not a class
...
class GameState(val randomGen : RandomGenerator, val board : Map[Point, CellType],val currentPiece : Tetronimo,val newPieceFlag : Boolean = false){
    ...
    def newPiecePlacement() : GameState = {
        ...
        val basePiece : Tetronimo = new Tetronimo(allPieces(pieceIndex)._1, allPieces(pieceIndex)._2)
        ...
        return new GameState(randomGen, newBoard, piece, false)
    }
    ...
    def newGameState(board : Map[Point,CellType], newPiece : Tetronimo , newPieceFlag : Boolean = newPiecFlag) : GameState = {
        return new GameState(randomGen, board, newPiece ,newPieceFlag)
    }
}
```
