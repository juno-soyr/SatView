...
### scala.reflect.internal.FatalError: 
  ThisType(method containsPoint) for sym which is not a class
  ...
  last tree to typer: Ident(p)
       tree position: line 24 of file://<HOME>/Uni/2024-2025/Period%201/OOFP/OOFP-Tetris-2024/src/tetris/logic/GameState.scala
            tree tpe: <error>
              symbol: value <error: <none>>
   symbol definition: val <error: <none>>: <error> (a TermSymbol)
      symbol package: tetris.logic
       symbol owners: value <error: <none>> -> value startingPosition -> class GameState
           call site: <none> in <none>

== Source file context for tree position ==

    21 
    22     val pieceIndex : Int = randomGen.randomInt(7)
    23     val piece : Tetronimo = new Tetronimo(allPieces(pieceIndex)._1, allPieces(pieceIndex)._2)
    24     val startingPosition : List[Point] = piece.struct.map(Point_CURSOR_(x + 1, p.y))
    25     
    26     
    27 
...
class GameState(val randomGen : RandomGenerator, val board : Map[(Int,Int), CellType], val currentPosition : List[Point] = List(), val pieceType : CellType = Empty,val newPiece : Boolean = false){
    ...
    val startingPosition : List[Point] = piece.struct.map(Point@@(x + 1, p.y))
    ...
    def newGameState(board : Map[(Int, Int),CellType], currentPosition : List[Point] = currentPosition ,pieceType : CellType = piece.blockType, newPiece : Boolean = newPiece) : GameState = {
