...
== Source file context for tree position ==
    ...
    46     }.toMap
    47 
    48     def newGameState(board : Map[(Int, Int),CellType], currentPosition : List[Point] = List(), pieceOnFloor : Boolean = _CURSOR_pieceOnFloor) : GameState = {
    49         return new GameState(randomGen, board, currentPosition, pieceOnFloor)
    50     }
    51 }
...
class GameState(val randomGen : RandomGenerator, val board : Map[(Int,Int), CellType], val currentPosition : List[Point] = List(), val newPiece : Boolean = false){
    ...
    def newGameState(board : Map[(Int, Int),CellType], currentPosition : List[Point] = List(), pieceOnFloor : Boolean = @@pieceOnFloor) : GameState = {
        return new GameState(randomGen, board, currentPosition, pieceOnFloor)
    }
}
```
