...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Ident(Empty)
       tree position: line 26 of file://<HOME>/Uni/2024-2025/Period%201/OOFP/OOFP-Tetris-2024/src/tetris/logic/GameState.scala
...
== Source file context for tree position ==
    ...
    23                 println(key)
    24                 key -> _CURSOR_piece.blockType
    25             }else{
    26                 key -> Empty
    27             }
    28         }.toMap
...
class GameState(val randomGen : RandomGenerator, val board : Map[Point, CellType], val currentPosition : List[Point] = List(), val pieceType : CellType = Empty,val newPiece : Boolean = false){
    ...
    val newBoard : Map[Point, CellType] = board.map {
        case (key, value) =>
            if(pointInPiece(currentPosition,key)){
                println(key)
                key -> @@piece.blockType
            }else{
                key -> Empty
    ...
    def newPiecePlacement() : GameState = {
        ...
        val startingPosition : List[Point] = piece.struct.map(p => Point(p.x + 4, p.y))
