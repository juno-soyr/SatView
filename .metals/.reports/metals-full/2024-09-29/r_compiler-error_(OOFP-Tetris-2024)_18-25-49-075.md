...
### scala.reflect.internal.FatalError: 
  ...
  last tree to typer: Ident(ne_CURSOR_w)
...
== Source file context for tree position ==
    ...
    46     }.toMap
    47 
    48     def newGameState(board : Map[(Int, Int),CellType], currentPosition : List[Point] = List(), pieceOnFloor : Boolean = ne_CURSOR_w) : GameState = {
...
class GameState(val randomGen : RandomGenerator, val board : Map[(Int,Int), CellType], val currentPosition : List[Point] = List(), val newPiece : Boolean = false){
    ...
    def newGameState(board : Map[(Int, Int),CellType], currentPosition : List[Point] = List(), pieceOnFloor : Boolean = ne@@w) : GameState = {
